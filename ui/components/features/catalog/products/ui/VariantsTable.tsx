...
export function VariantsTable(props: {
  ...
  const cols: Array<VirtualColumnDef<ProductVariantDTO>> = [
    {
      key: "img",
      header: "Img",
    ...
    },
    {
      ...
      className: "col-span-2",
      render: (v) => (
        <span className="text-sm truncate block">{displayVariantTitle(v.title, v.sku)}</span>
      ),
    },
    {
      key: "price",
    ...
    },
    {
      ...
      render: (v) => (
        <div
          ...
          // ✅ evita que el click de fila abra el dialog
  ...
  return (
    <VirtualDataTable<ProductVariantDTO>
      ...
      onRowClick={(v) => props.onEdit(v)} // ✅ fila clickable abre dialog
      getRowClassName={(row) => ["cursor-pointer", !row.isActive ? "opacity-70" : ""].filter(Boolean).join(" ")}
    />
  );
}