...
export function WarehousesScreen() {
  ...
  // ✅ para resaltar la fila abierta/seleccionada
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">Gestiona warehouses del store actual (contexto por terminal).</p>
        </div>
      ...
      <Card>
        ...
        <CardContent className="grid gap-3">
          <TriStateFilterBar
            ...
            onSearchSubmit={() => void vm.submitSearch()}
            searchButtonText="Buscar"
            placeholder="Buscar por nombre, código o ubicación…"
            filter={vm.filter}
            onFilterChange={(v) => void vm.setFilterAndRefresh(v)}
            counts={vm.counts}
            busy={vm.loading}
          />
          ...
          <WarehousesTable
            ...
            onRowClick={(w) => vm.openEdit(w)} // ✅ click fila = abrir dialog
          />
        </CardContent>
      </Card>
